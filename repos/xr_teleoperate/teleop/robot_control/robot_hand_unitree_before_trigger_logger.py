...
class Dex3_1_Controller:
    def __init__(
        ...
        left_trigger_value_in=None,
        right_trigger_value_in=None,
        use_trigger_mapping=False,
    ):
        """
    ...
    use_trigger_mapping=False,
        ...
        # dex3_left_close_delta: ændringer i joint-positioner når trigger går fra 0 (åben) til 1 (lukket).
        # Array indeks: [thumb0, thumb1, thumb2, middle0, middle1, index0, index1]
        # left_open_pose er sat til den åbne hånd's grænsepositioner fra URDF.
        # left_close_delta er forskellen fra åben position til lukket position.
        dex3_left_close_delta = np.array([0.0, 0.920, 1.74532925, -1.57079632, -1.74532925, -1.57079632, -1.74532925])

        # dex3_right_close_delta: ændringer i joint-positioner når trigger går fra 0 (åben) til 1 (lukket).
        # Array indeks: [thumb0, thumb1, thumb2, index0, index1, middle0, middle1]
        # right_open_pose er sat til den åbne hånd's neutralpositioner.
        # right_close_delta er lukke-positioner ift. åben (0). Thumb0 bevares, men thumb1/2 lukker.
        dex3_right_close_delta = np.array([0.0, -0.920, -1.74532925, 1.57079632, 1.74532925, 1.57079632, 1.74532925])

        left_open_pose = np.zeros(Dex3_Num_Motors)
        right_open_pose = np.zeros(Dex3_Num_Motors)
        ...
        try:
            while self.running:
                ...
                if use_trigger_mapping and left_trigger_value_in is not None and right_trigger_value_in is not None:
                    ...
                    # Normalize XR trigger values from TeleData: 10 = released, 0 = fully pressed.
                    left_trigger = np.clip(1.0 - left_trigger / 10.0, 0.0, 1.0)
                    right_trigger = np.clip(1.0 - right_trigger / 10.0, 0.0, 1.0)
...
class Dex1_1_Gripper_Controller:
    ...
    use_trigger_mapping=False,
        ...
        self.gripper_control_thread = threading.Thread(target=self.control_thread, args=(left_trigger_value_in, right_trigger_value_in, self.left_gripper_state_value, self.right_gripper_state_value,
