...
class HandRetargeting:
    def __init__(self, hand_type: HandType):
        ...
        try:
            ...
            left_cfg = self.cfg["left"].copy()
            right_cfg = self.cfg["right"].copy()

            for key in (
                "target_link_human_indices_dexpilot",
                "target_link_human_indices_vector",
            ):
                left_cfg.pop(key, None)
                right_cfg.pop(key, None)

            left_retargeting_config = RetargetingConfig.from_dict(left_cfg)
            right_retargeting_config = RetargetingConfig.from_dict(right_cfg)
